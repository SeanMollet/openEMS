...
Possible performance improvements for the CUDA backend of the GPU engine
(`--engine=gpu`, `FDTD/cuda/`). Many ideas are shared with the Metal backend
...
There is no dedicated benchmark of the CUDA backend yet. The figures below are
...
- On small meshes it dominates: a 100k-cell timestep takes ~33 µs of GPU work,
  so the GPU mostly waits for the host. Together with the CUDA context creation
...
- CUDA context creation takes a significant fraction of a second, and
  `GPU_Backend_CUDA::New()` triggers it only when the engine is created, after
  the operator setup. Starting the context earlier on a background thread (for
  example with `cudaFree(0)` when the GPU engine is selected) would overlap it
  with the operator setup.
- `GPU_Backend_CUDA::Init()` gathers the coefficients through four virtual
