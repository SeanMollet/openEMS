...
#ifndef GPU_BACKEND_CUDA_H
#define GPU_BACKEND_CUDA_H
...
/*!
  ...
  the grids of a simulation run in order on one CUDA stream (see
  NewSubGridBackend()), which is only synchronized when the host needs the fields.

  This header is plain C++, the CUDA state is kept in Impl (see cuda_internal.h).
  */
class GPU_Backend_CUDA : public GPU_Backend
{
public:
	//! Create the backend on the current CUDA device, or return NULL if there is none
	static GPU_Backend_CUDA* New();
	virtual ~GPU_Backend_CUDA();
