...
{
	const unsigned int z = B.nz + blockIdx.x*blockDim.x + threadIdx.x;
...
void GPU_Backend_CUDA::Impl::MainRange(CUDA_GridDim& B, CUDA_GridDim& E) const
{
	B = main_start;
	E = main_stop;
	if (zslab_lo)
...
void GPU_Backend_CUDA::UpdateVoltages()
{
	...
	CUDA_GridDim B, E;
	d->MainRange(B, E);
	CUDA_Launch(d, "update_voltages", update_voltages, E.nz-B.nz, E.ny-B.ny, E.nx-B.nx,
...
void GPU_Backend_CUDA::UpdateCurrents()
{
	...
	CUDA_GridDim B, E;
	d->MainRange(B, E);
	CUDA_Launch(d, "update_currents", update_currents, E.nz-B.nz, E.ny-B.ny, E.nx-B.nx,
