...
// Nodes below Z.z_lo or from Z.z_hi on belong to UPML regions along z (see CUDA_ZSlabs).
__global__ void update_voltages(float* __restrict__ volt, const float* __restrict__ curr,
                                ...
                                CUDA_GridDim N, CUDA_GridDim B, CUDA_GridDim E, CUDA_ZSlabs Z)
{
	...
	const CUDA_MainCoeff C = main_coeff(index, ca, cb, mode, sn, i, 0);
	const float* vv = C.a;
	const float* vi = C.b;
                                ...
                                CUDA_GridDim N, CUDA_GridDim B, CUDA_GridDim E, CUDA_ZSlabs Z)
{
	...
	const CUDA_MainCoeff C = main_coeff(index, ca, cb, mode, sn, i, 6);
	const float* ii = C.a;
	const float* iv = C.b;
	const unsigned int s = C.s;
	float c;
...
#define DFT_WEIGHTS 32
struct CUDA_DFTWeights { float2 w[DFT_WEIGHTS]; };

__global__ void field_dft(const float* __restrict__ f, const GPU_GatherEntry* __restrict__ entries, unsigned int n,
                          CUDA_DFTWeights W, unsigned int nw, float2* __restrict__ sums)
{
	const unsigned int i = blockIdx.x*blockDim.x + threadIdx.x;
	if (i>=n)
		return;
...
struct KernelTimeTable
{
	...
	~KernelTimeTable()
	{
		...
		fprintf(stderr, "CUDA kernel times (%.1f ms total):\n", total);
...
bool CUDA_KernelTimes::Enabled()
{
	static const bool enabled = getenv("OPENEMS_CUDA_KERNEL_TIMES") && (atoi(getenv("OPENEMS_CUDA_KERNEL_TIMES"))!=0);
	return enabled;
}

void CUDA_KernelTimes::Begin(cudaStream_t stream)
{
...
void CUDA_KernelTimes::End(cudaStream_t stream, const char* name)
{
	KernelTimeTable& t = KernelTimes();
...
void GPU_Backend_CUDA::Impl::MainRange(CUDA_GridDim& B, CUDA_GridDim& E, unsigned int& threads_z)
{
	B = main_start;
	E = main_stop;
	if (zslab_lo)
	...
	{
		auto warps = [](unsigned int n) {return (n+31)/32*32;};
...
void GPU_Backend_CUDA::UpdateVoltages()
{
	...
	{
		// the voltages and currents of the main nodes, see update_fused
		// the main nodes, or the whole grid with the UPML regions in the kernel
		CUDA_GridDim B = d->main_start;
		CUDA_GridDim E = d->main_stop;
		if (d->fregions.count)
		{
			B.nx = B.ny = B.nz = 0;
			E = d->dim;
	...
	CUDA_Launch(d, "update_voltages", update_voltages, threads_z, E.ny-B.ny, E.nx-B.nx,
...
void GPU_Backend_CUDA::UpdateCurrents()
{
	...
	unsigned int threads_z;
	d->MainRange(B, E, threads_z);
	CUDA_Launch(d, "update_currents", update_currents, threads_z, E.ny-B.ny, E.nx-B.nx,
...
int GPU_Backend_CUDA::AddFieldDFT(const std::vector<GPU_GatherEntry>& entries, bool currents, unsigned int count)
{
	...
	catch (std::runtime_error&)
	{
		...
		cudaGetLastError();
		std::cerr << "GPU_Backend_CUDA: not enough memory for a frequency domain dump on the device" << std::endl;
		return -1;
	}
...
void GPU_Backend_CUDA::AccumulateFieldDFT(int id, const std::vector<std::complex<float>>& weights)
{
	...
	for (unsigned int k0=0; k0<count; k0+=DFT_WEIGHTS)
	{
		...
		CUDA_Launch(d, "field_dft", field_dft, dft.n, 1, 1, (const float*)(dft.currents ? d->curr : d->volt),
		            (const GPU_GatherEntry*)dft.entries, dft.n, W, nw, dft.sums + (size_t)k0*dft.n);
	}
}

bool GPU_Backend_CUDA::ReadFieldDFT(int id, std::vector<std::complex<float>>& sums)
{
	if ((id<0) || ((size_t)id>=d->field_dfts.size()))
		return false;
	const Impl::FieldDFT& dft = d->field_dfts[id];
	sums.resize((size_t)dft.n*dft.count);
	CUDA_Check(cudaMemcpyAsync(sums.data(), dft.sums, sums.size()*sizeof(float2), cudaMemcpyDeviceToHost, d->Stream()), "download");
	d->Flush();
	return true;
}
...
{
	E_energy = H_energy = 0;
	CUDA_GridDim L = {numNodes[0], numNodes[1], numNodes[2]};
	...
	CUDA_Launch(d, "energy_lines", energy_lines, L.nz, L.ny, 1, (const float*)d->volt, (const float*)d->curr, d->energy, d->dim, L);
	CUDA_Check(cudaMemcpyAsync(d->energy_host.data(), d->energy, count*sizeof(float2), cudaMemcpyDeviceToHost, d->Stream()), "energy download");
	d->Flush();
...
static const CUDA_ExtensionFactory CUDA_EXTENSIONS[] = {
	...
	CUDA_CreateExt_TFSF,
	CUDA_CreateExt_Absorbing_BC,
	CUDA_CreateExt_SteadyState,
	CUDA_CreateExt_Cylinder,
};
