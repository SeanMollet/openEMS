...
__global__ void upml_fused_volt(float* volt, const float* curr, UPMLFusedArgs A, CUDA_GridDim N, UPMLRegion R)
{
	UPML_THREAD
...
__global__ void upml_fused_curr(float* curr, const float* volt, UPMLFusedArgs A, CUDA_GridDim N, UPMLRegion R)
{
	UPML_THREAD
	const unsigned int gx = R.sx + x, gy = R.sy + y, gz = R.sz + z;
	const unsigned int sn = N.nx*N.ny*N.nz;
...
protected:
	...
	{
		CUDA_Launch(d, "upml_pre", upml_pre, m_Region.lz, m_Region.ly, m_Region.lx, field, flux, c_old, c_fo, d->dim, m_Region);
	}
	void Post(float* field, float* flux, const float* c_fn)
	{
		CUDA_Launch(d, "upml_post", upml_post, m_Region.lz, m_Region.ly, m_Region.lx, field, flux, c_fn, d->dim, m_Region);
	}
	void FuseVolt()
	{
		UPMLFusedArgs A = Args(m_VoltFlux, m_VV, m_VVFO, m_VVFN, d->vv, d->vi);
		CUDA_Launch(d, "upml_fused_volt", upml_fused_volt, m_Region.lz, m_Region.ly, m_Region.lx, d->volt, (const float*)d->curr, A, d->dim, m_Region);
	}
	void FuseCurr()
	{
		UPMLFusedArgs A = Args(m_CurrFlux, m_II, m_IIFO, m_IIFN, d->ii, d->iv);
		CUDA_Launch(d, "upml_fused_curr", upml_fused_curr, m_Region.lz, m_Region.ly, m_Region.lx, d->curr, (const float*)d->volt, A, d->dim, m_Region);
	}
...
bool CUDA_Ext_UPML::Fused()
{
	if (d->upml_fused<0)
	{
		...
		if (d->upml_fused)
			std::cout << "GPU_Backend_CUDA: " << d->upml.size() << " UPML regions fused with the main updates" << std::endl;
	}
