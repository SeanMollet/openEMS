...
__global__ void upml_pre(float* field, float* flux, const float* c_old, const float* c_fo, CUDA_GridDim N, UPMLRegion R)
{
	UPML_THREAD
...
__global__ void upml_post(float* field, float* flux, const float* c_fn, CUDA_GridDim N, UPMLRegion R)
{
	UPML_THREAD
	for (unsigned int n=0; n<3; ++n)
	{
...
__global__ void upml_fused_volt(float* volt_out, const float* volt_in, const float* curr, UPMLFusedArgs A, CUDA_GridDim N, UPMLRegion R)
{
	UPML_THREAD
	upml_fused_volt_node(volt_out, volt_in, curr, A, N, R, x, y, z);
}

__global__ void upml_fused_curr(float* curr_out, const float* curr_in, const float* volt, UPMLFusedArgs A, CUDA_GridDim N, UPMLRegion R)
{
	UPML_THREAD
	upml_fused_curr_node(curr_out, curr_in, volt, A, N, R, x, y, z);
}

class CUDA_Ext_UPML : public GPU_Extension
{
public:
	CUDA_Ext_UPML(GPU_Backend_CUDA::Impl* impl, Operator_Ext_UPML* op_ext, Engine* eng);
	virtual ~CUDA_Ext_UPML();
...
protected:
	...
	{
		CUDA_Launch(d, "upml_post", upml_post, m_Region.lz*m_Region.ly, 1, m_Region.lx, field, flux, c_fn, d->dim, m_Region);
	}
	...
	{
		CUDA_Launch(d, "upml_fused_volt", upml_fused_volt, m_Region.lz*m_Region.ly, 1, m_Region.lx, volt_out, (const float*)d->volt, (const float*)d->curr, VoltArgs(), d->dim, m_Region);
	}
	void FuseCurr(float* curr_out)
	{
		CUDA_Launch(d, "upml_fused_curr", upml_fused_curr, m_Region.lz*m_Region.ly, 1, m_Region.lx, curr_out, (const float*)d->curr, (const float*)d->volt, CurrArgs(), d->dim, m_Region);
	}
	...
	float* m_VoltFlux2;   //!< ... and the second buffer of the voltage flux
...
CUDA_Ext_UPML::CUDA_Ext_UPML(GPU_Backend_CUDA::Impl* impl, Operator_Ext_UPML* op_ext, Engine* eng)
{
	d = impl;
	m_Eng = eng;
	m_InMain = false;
...
CUDA_Ext_UPML::~CUDA_Ext_UPML()
{
	...
	for (size_t r=0; r<d->upml.size(); ++r)
		static_cast<CUDA_Ext_UPML*>(d->upml[r])->m_InMain = false;
}

bool CUDA_Ext_UPML::Fused()
{
	if (d->upml_fused<0)
	{
		d->upml_fused = CanFuse();
		if (d->upml_fused)
			std::cout << "GPU_Backend_CUDA: " << d->upml.size() << " UPML regions fused with the main updates ("
...
bool CUDA_Ext_UPML::CanFuse()
{
	...
	CUDA_FusedRegions& F = d->fregions;
	F.count = 0;
	F.lo = d->main_start;
	F.hi = d->main_stop;
	bool sides_ok = true;
...
GPU_Extension* CUDA_CreateExt_UPML(GPU_Backend_CUDA::Impl* d, Engine_Extension* eng_ext, Engine* eng)
{
	...
	if (!op_ext)
		return NULL;
	return new CUDA_Ext_UPML(d, op_ext, eng);
}
