...
public:
	...
	virtual void DoPostVoltageUpdates() {if (Fused()) {if (!m_InMain) FuseVolt();} else Post(d->volt, m_VoltFlux, m_VVFN);}
	virtual void DoPreCurrentUpdates()  {if (!Fused()) Pre(d->curr, m_CurrFlux, m_II, m_IIFO);}
	virtual void DoPostCurrentUpdates() {if (Fused()) {if (!m_InMain) FuseCurr();} else Post(d->curr, m_CurrFlux, m_IIFN);}
...
protected:
	...
	void FuseVolt()
	{
		CUDA_Launch(d, "upml_fused_volt", upml_fused_volt, m_Region.lz*m_Region.ly, 1, m_Region.lx, d->volt, (const float*)d->curr, VoltArgs(), d->dim, m_Region);
	}
	void FuseCurr()
	{
		CUDA_Launch(d, "upml_fused_curr", upml_fused_curr, m_Region.lz*m_Region.ly, 1, m_Region.lx, d->curr, (const float*)d->volt, CurrArgs(), d->dim, m_Region);
	}
