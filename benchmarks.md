...
- a re-entrant coaxial cavity in a closed PEC can, 33 x 33 x 66 = 71874 cells,
- three runs: the bare gap, then a 3 pF and a 6 pF capacitor across it,
- the bare cavity stops at 271410 timesteps on the end criteria, the loaded two
  run the full 400000,
- every machine gave the same resonances: 1.0744, 0.7060 and 0.5494 GHz.
...
The 6 pF run, which is 400000 timesteps on every machine.

| Machine | Host CPU | MCells/s | Timestepping |
|---|---|---|---|
| Apple M5 Max, CPU (multithreaded) | Apple M5 Max | 615 | 46.7 s |
| i7-12700K, CPU (multithreaded) | i7-12700K | 784 | 36.7 s |
| Apple M5 Max, GPU (Metal) | Apple M5 Max | **4428** | **6.5 s** |
| RTX 3060 Ti | i7-12700K | 3965 | 7.3 s |
| AMD Radeon 760M (HIP) | Ryzen 5 7640HS | 856 | 33.6 s |

The order is not the one of the other tests. The M5 Max is ahead of the RTX 3060
Ti here and behind it on the horn, and the Radeon 760M, which is about four times
its host CPU on the horn, is within 10 % of a desktop CPU here: 400000 launches
of a kernel with little work in it is what this measures.
