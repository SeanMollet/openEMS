# -*- coding: utf-8 -*-
"""
 Horn antenna benchmark -- the timestepping speed of the engines on a real model.

 usage: Horn_Antenna_Benchmark.py [td|fd] [engine]

 The horn antenna tutorial (python/Tutorials/Horn_Antenna.py) run as a benchmark:
 the geometry viewer stays closed, the NF2FF box records either the time domain
 fields ("td", 4.7 GB of HDF5) or the 15 GHz fields during the run ("fd", 12 MB),
 and the end criteria is evaluated on a fixed schedule so that every machine
 stops at the same timestep and the numbers compare. See benchmarks.md.
...
### Benchmark arguments
NF2FF_DOMAIN = sys.argv[1] if len(sys.argv) > 1 else 'td'
ENGINE       = sys.argv[2] if len(sys.argv) > 2 else 'multithreaded'
if NF2FF_DOMAIN not in ('td', 'fd'):
    sys.exit('usage: Horn_Antenna_Benchmark.py [td|fd] [engine]')
...
# "fd": the box records the 15 GHz fields during the run instead of dumping the
# time domain fields, which is the same far field out of 12 MB instead of 4.7 GB
if NF2FF_DOMAIN == 'fd':
    nf2ff = FDTD.CreateNF2FFBox(frequency=[f0])
else:
    nf2ff = FDTD.CreateNF2FFBox()

### Optional: write XML and launch AppCSXCAD for geometry inspection
if 0:   # benchmark: do not launch AppCSXCAD
...
if not post_proc_only:
    # exact_endcriteria: the energy end criteria is otherwise re-evaluated on a
    # wall-clock schedule, so a faster machine runs further before it stops and
    # the runs are not comparable.
    FDTD.Run(Sim_Path, cleanup=True, engine=ENGINE, exact_endcriteria=True)
